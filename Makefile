...
# Set the KBUILD_BUILD_VERSION as the XLDK version
export XLDK_VERSION ?= 4.2.1
export KBUILD_BUILD_VERSION := Broadcom XLDK-$(XLDK_VERSION)
...
link-source: $(BUILD_DIR)
	@if [ "$(LINUX_SITE_METHOD)" = "local" ]; then \
		ln -snf $(TOPDIR)/$(LINUX_SITE) $(BUILD_DIR)/linux-$(LINUX_VERSION); \
		touch $(TOPDIR)/$(LINUX_SITE)/.stamp_rsynced; \
		ln -snf $(BUILD_DIR)/linux-$(LINUX_VERSION) $(BUILD_DIR)/linux; \
	elif [ "$(LINUX_SITE_METHOD)" != "" ]; then \
		ln -snf $(BUILD_DIR)/linux-$(LINUX_VERSION) $(BUILD_DIR)/linux; \
	fi;
	@if [ "$(UBOOT_SITE_METHOD)" = "local" ]; then \
		ln -snf $(TOPDIR)/$(UBOOT_SITE) $(BUILD_DIR)/uboot-local; \
		touch $(TOPDIR)/$(UBOOT_SITE)/.stamp_rsynced; \
		ln -snf ./uboot-local $(BUILD_DIR)/uboot; \
	elif [ "$(UBOOT_SITE_METHOD)" != "" ]; then \
		ln -snf ./uboot-$(UBOOT_VERSION) $(BUILD_DIR)/uboot; \
	fi;

dirs: $(BUILD_DIR) $(STAGING_DIR) $(TARGET_DIR) \
	$(HOST_DIR) $(BINARIES_DIR) link-source
...
%_defconfig: $(BUILD_DIR)/buildroot-config/conf $(TOPDIR)/configs/brcm_%_defconfig outputmakefile
	@$(COMMON_CONFIG_ENV) BR2_DEFCONFIG=$(TOPDIR)/configs/$@ \
		$< --defconfig=$(TOPDIR)/configs/brcm_$@ $(CONFIG_CONFIG_IN)
...
clean:
	@if [ "$(LINUX_SITE_METHOD)" = "local" ]; then \
		make -C $(TOPDIR)/$(LINUX_SITE) distclean; \
		rm -f $(TOPDIR)/$(LINUX_SITE)/.stamp*; \
	fi;
	@if [ "$(UBOOT_SITE_METHOD)" = "local" ]; then \
		make -C $(TOPDIR)/$(UBOOT_SITE) distclean; \
		rm -f $(TOPDIR)/$(UBOOT_SITE)/*_config; \
		rm -f $(TOPDIR)/$(UBOOT_SITE)/.stamp*; \
	fi;
